...
- Export Runtime Data writes one deterministic `.kworld` version-3 game-development package: a self-describing JSON manifest, dense four-byte terrain records, dense per-tile Resource and Season span indexes, and compact Resource and Season occurrence records. Authoring locks, rules, recipes, support fields, and diagnostics remain editor-only.
