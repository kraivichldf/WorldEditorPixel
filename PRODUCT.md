...
Create, optionally generate, inspect, stamp, save, and reopen deterministic campaign tiles. Each tile owns one base terrain type, an optional safe custom-land identity, one centre height, zero or more orthogonal resource occurrences, and exactly one static Tile Season ID plus its authoring lock; adjacent centre heights automatically form a continuous surface. Generated terrain, Resources, and Seasons become the same editable authority as manual stamps after explicit preview acceptance. The editor succeeds when terrain identity, centre height, resource potential/lock authority, Tile Season identity/lock authority, and the interpolation contract survive a save/load roundtrip unchanged.
...
- Terrain, Resources, and Seasons are explicit workspaces over the same canvas, pan/zoom transform, campaign grid, hover coordinate, and right-click pin. The Resources rail filters the built-in/custom catalog, writes exact per-resource potential `1..100` over an independent `1 × 1` through `25 × 25` complete-tile area, removes only the selected ID, and locks manual edits by default. The Seasons rail paints, resets, locks, or unlocks one complete static Season identity per tile without any month or clock.
...
- A protected Custom seasons manager owns Spring/Summer/Autumn/Winter plus safe custom definitions, portable fallbacks, explicit first-match priority, and one unconditional final Catch-all. Preview-first Season regeneration compares unchanged Current authority with an unapplied Candidate, preserves locks/out-of-scope tiles, installs only the exact current Candidate, and retains immutable climate support/fingerprints for pinned inspection; winning and shadowed rules are re-evaluated from the current catalog.
- The Seasons view preserves terrain context under one categorical color per complete tile. Exact identity labels, lock markers, and optional boundary blending are presentation-only; Season strokes share the same ordered terrain/resource history.
...
- Save/reopen uses one editor-level staged coordinator for terrain, optional sparse Resource sidecars, and complete Season definition/layer sidecars. A project is marked clean only after the combined candidate reload, all-authority revision checks, and managed-file commit succeed.
- Export Runtime Data writes one deterministic `.kworld` version-3 game-development package: a self-describing JSON manifest, dense four-byte terrain records, a dense per-tile Resource index, compact Resource occurrence records, and one dense Season catalog index per tile. Authoring locks, rules, recipes, support fields, and diagnostics remain editor-only.
...
- Terrain/resource rendering scales with visible pixels and sparse non-default data by default. The complete Season authority, Season generation support, Season persistence, and runtime Season export intentionally scale with the full logical tile grid, bounded at `250,000` tiles.
...
- Static Tile Season is a peer authority, not a calendar simulation or an encoding inside terrain/resource values. Generation consumes terrain but never resources and never rewrites either.
- Additional world layers remain explicit consumers or peers, not encodings inside height, resource potential, or Tile Season identity.
...
ADR-0030 Slices 1-7 are implemented in the running product: complete static Tile Season authority, protected built-in/custom catalog and priority, manual Seasons workspace, shared-history full-cell painting/locks, Earth-like immutable terrain support and first-match generation, strict sidecars, deterministic runtime package version 3, Current/Candidate regeneration, exact acceptance, cached pinned support/rule/staleness diagnostics, atomic generated-new-world Terrain-and-Season composition, explicit blank-world defaults, reviewed changed-lattice physical-overlap lock remapping, native normal/narrow and keyboard verification, maximum-grid diagnostics, and a smoke-tested self-contained Windows build. Continue from [[docs/Decisions/ADR-0030 - Static Preview-First Campaign Season Layer|ADR-0030]] and [[docs/Reference/Campaign Season Layer Plan|Campaign Season Layer Plan]].
