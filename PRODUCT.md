...
Create, optionally generate, inspect, stamp, save, and reopen deterministic campaign tiles. Each tile owns one base terrain type, an optional safe custom-land identity, one centre height, and zero or more orthogonal resource occurrences; adjacent centre heights automatically form a continuous surface. Generated terrain starts immediately become the same editable tiles as manual stamps. The editor succeeds when terrain identity, centre height, resource potential/lock authority, and the interpolation contract survive a save/load roundtrip unchanged.
...
Users work in long desktop sessions with a central world canvas, persistent stamp and inspector rails, mouse-centred zoom, middle-button pan, full-cell drag editing, and keyboard undo/redo. Version-2 projects are local folders containing explicit metadata and sparse campaign-tile records.
...
- Terrain and Resources are explicit workspaces over the same canvas, pan/zoom transform, campaign grid, hover coordinate, and right-click pin. The Resources rail filters the built-in/custom catalog, writes exact per-resource potential `1..100` over an independent `1 × 1` through `25 × 25` complete-tile area, removes only the selected ID, and locks manual edits by default.
...
- Save/reopen uses one editor-level staged coordinator for terrain files plus optional sparse resource sidecars. A project is marked clean only after the combined candidate reload, revision checks, and managed-file commit succeed.
- Export Runtime Data writes one deterministic `.kworld` version-2 game-development package: a self-describing JSON manifest, dense four-byte terrain records, a dense per-tile resource index, and compact resource occurrence records. Authoring locks and diagnostics remain editor-only.
...
- Costs scale with visible pixels and non-default tiles, not the full theoretical grid by default.
...
- Additional world layers remain explicit consumers or peers, not encodings inside height or resource potential.

## Accessibility & Inclusion

Use keyboard-accessible standard desktop controls, visible focus, sufficient contrast, text labels alongside terrain/resource colors, high-contrast outlined elevation and resource-potential numbers, an outlined pinned selection, and direct validation/recovery messages. Resource lock and suitability states must include text rather than depend on color. Map stamping remains mouse-led in this milestone; keyboard tile navigation and stamping are future accessibility work.
