...
World.Core (campaign world, interpolation, commands, validation, files)
        |
        v
Version-2 project folder (terrain files + optional sparse resource sidecars)
```
...
**Use this world** passes the exact temporary `CampaignWorld` to `EditorViewModel.RegenerateWorld`. The view model rejects Blank, swaps the definition and campaign tile map together, clears history and pointer/pinned state, refreshes custom palette entries, clamps the active stamp height into the accepted limits, preserves project/import identity, and marks the document modified. Cancelling or closing leaves the current definition, tile map, and undo history untouched. See [[../Decisions/ADR-0015 - Preview-First Current World Regeneration|ADR-0015]].
...
Resources mode reuses the same pointer lifecycle through `CampaignResourceStrokeBuilder`. Add/update writes only the selected ID's potential/lock over a clipped `CampaignTileArea`; erase removes only that ID. Release records one already-applied `CampaignResourceEditCommand`, while Escape restores the stroke. `EditorViewModel` owns terrain, the compatible resource map, nullable resource settings, and one `CommandHistory`, so terrain/resource commands interleave in exact LIFO order. Workspace/filter/zoom/pin changes remain view state and do not dirty the document.
...
`CampaignResourceProjectSerializer` owns the optional custom-definition, generation-settings, and sparse occurrence sidecars. `CampaignEditorProjectSerializer` is the editor lifecycle boundary: save serializes both authorities into a unique sibling staging directory, reloads the staged candidate, verifies captured terrain/resource revisions, then commits only the six known managed files with backups and rollback for ordinary I/O failure. Stale optional sidecars are removed, post-commit staging cleanup is non-fatal, and `MarkSaved` runs only after success. Open similarly loads terrain then resources and replaces the visible document only when the complete candidate is valid. Missing resource sidecars produce the built-in catalog, null settings, and an empty map; legacy imports never attach sibling resources.
...
`CampaignWorldRuntimeExporter` is a one-way engine-neutral handoff boundary. The editor always calls its resource-aware version-2 overload. It retains byte-compatible row-major four-byte `tiles.bin` records, adds an eight-byte dense per-tile `resource-index.bin`, compact four-byte `resource-records.bin`, and a manifest resource catalog. Authoring locks, rules, warnings, symbols, and generation settings do not enter runtime data. Bounded buffers, stable catalog/occurrence ordering, fixed ZIP timestamps, revision checks, and SHA-256 values make equal worlds produce equal package bytes without mutating either authority.
