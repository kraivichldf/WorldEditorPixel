...
`CampaignSeasonMap` owns zero or more Season Occurrences at each valid campaign coordinate. One tile may contain several stable Season IDs but at most one occurrence of a given ID, and each occurrence has its own lock flag. The map is sparse: absence means that Season ID does not occur there. Spring, Summer, Fall, and Winter are protected built-ins; custom definitions carry a portable built-in fallback, appearance, and environmental rule. Catalog order is presentation/serialization order only. The persisted `KWSEASON` sidecar uses a dense tile-span index plus compact occurrence records, while the accepted generation recipe remains optional metadata rather than occurrence provenance.

`CustomSeasonsDialog` edits detached catalog and enabled-generation drafts. Existing stable IDs and built-in identity are protected; new custom drafts remain manual-only until explicitly enabled. A referenced custom deletion requires an explicit replacement for its occurrences and preserves their lock flags; replacing one ID never removes other IDs on the same tile. `EditorViewModel.UpdateSeasons` validates and constructs the complete replacement map before one atomic swap, clears shared history only for an effective authority change, and leaves an equivalent apply as a no-op.
...
New World can remain Blank or call the engine-neutral `CampaignMapGenerator`. The generator consumes the validated definition plus preset, seed, terrain style, Mountain-system profile, hydrology amount, directional coastline style, tidal-inlet amount, optional `CampaignMapLandMix`, and optional safe custom terrain definitions. For a generated preset, the dialog builds a temporary `CampaignWorld`, applies the complete ordered `CampaignTileEntry` list once through `CampaignTileMap.SetTiles`, then builds an empty Season Occurrence source over that terrain and runs `CampaignSeasonWorldRegenerator.GenerateNewWorld`. Terrain and Season rasters are separate bounded preview views; the impact well reports terrain counts and occurrence coverage for every definition independently. Input changes invalidate acceptance while retaining the old Candidate for comparison. **Use this world** passes the exact preview world, Season map, enabled definition set, saved recipe, and support fields to `EditorViewModel`; the main window does not regenerate either layer. The view model validates the complete tuple, clears history, marks the document unsaved, and enables ordinary painting. Blank bypasses procedural preview, creates untouched terrain and an empty Season Occurrence map, and invents no generation recipe.
...
World regeneration captures terrain, resource, and Season source revisions before worker execution. Same-lattice Season candidates preserve every occurrence and lock exactly. On a changed lattice, `CampaignSeasonWorldRegenerator` maps each locked source occurrence by its physical tile centre, merges only duplicate target identities with the same Season ID, allows different IDs to coexist at one target, and regenerates unlocked occurrences from the reviewed terrain. Only locked occurrences whose physical centres fall outside the replacement world block acceptance until the user explicitly permits those named drops. **Use this world** passes the exact temporary terrain/resource/Season tuple to `EditorViewModel.RegenerateWorld`; the view model rejects Blank and stale source/Candidate revisions, requires exact catalog/enabled-set/recipe identity and a ready drop report, then swaps every authority together, clears history and pointer/pinned state, refreshes selectors, clamps the active stamp height, preserves project/import identity, and marks the document modified. Cancelling, closing, failed generation, or an unpermitted drop leaves current authority and history untouched. See [[../Decisions/ADR-0015 - Preview-First Current World Regeneration|ADR-0015]] and [[../Decisions/ADR-0030 - Preview-First Campaign Season Occurrences|ADR-0030]].
...
Seasons mode uses the same lifecycle through `CampaignSeasonStrokeBuilder`. **Add selected** upserts only the selected stable ID with the explicit manual-lock choice; **Erase selected** removes only that identity; Lock/Unlock alter only that occurrence. Other Season IDs on the same tile remain exact. Every tool applies to the clipped complete-cell `CampaignTileArea`, one drag records one already-applied `CampaignSeasonEditCommand`, and Escape restores the stroke. Terrain, resources, Season filters, labels, boundary blending, workspace choice, zoom, and pin remain independent.
...
Seasons mode adds a third independently keyed bounded visible raster over the unchanged terrain surface. A tile composites the colors/tints of all its occurrences; optional boundary blending samples only neighbor presentation colors and never changes the sparse authority. At `28 px/tile`, outlined labels expose every configured identity and its lock state. Hover and pin list the complete Tile Season Set with exact IDs, fallbacks, locks, terrain/elevation, and independent rule summaries. After accepted generation, a cached immutable support/fingerprint projection supplies latitude, annual/warm/cold temperature, moisture, annual range/seasonality, rain shadow, exact water distances, and canonical source/input staleness. The inspector evaluates each selected definition independently and reports whether its current occurrence agrees with its rule; no winner or shadow outcome exists. A saved recipe rebuilds the support/fingerprint cache after reopen behind revision/reference guards.

`SeasonGenerationDialog` is a resource-independent preview boundary over the current lattice. It captures terrain and Season state on the owner thread, runs `CampaignSeasonGenerator` on the immutable snapshot away from the UI thread, and gives unchanged Current and unapplied Candidate canvases one dispatcher-safe synchronized viewport. A read-only whole-cell rectangle gesture defines inclusive scope without entering a paint stroke. In-dialog generation settings and scope stale the retained Candidate; report selection, grid, labels, blending, pan, zoom, and narrow Current/Candidate switching are display state only. Catalog/enabled-set management requires closing the modal, so that dialog Candidate is discarded. Acceptance is centralized in `EditorViewModel`: it rechecks both source revisions, value-equal definition, exact catalog/enabled set, Candidate revision, settings, and scope, then installs the exact candidate/recipe, clears shared history, and marks dirty while preserving terrain, resources, and project/import identity.
...
[[../Decisions/ADR-0030 - Preview-First Campaign Season Occurrences|ADR-0030]] owns the third peer authority: zero or more stable Season IDs per logical tile, each with its own lock, generated from an immutable terrain snapshot and saved Season Seed without calendar semantics. The implementation includes definitions, independent rules, catalog, enabled-generation settings, scope, sparse map/locks, atomic mutations, stable seeds/fingerprints, shared-history commands, version-2/version-3 terrain adapters, immutable capture, Earth-like support fields, independent overlapping generation, reports, strict sidecars, missing-sidecar empty compatibility, nine-file staged coordination, runtime package version 3 span/record streams, manual canvas editing, protected custom-definition management, Current/Candidate regeneration, exact acceptance, generation-backed pinned diagnostics, atomic generated-new-world composition, empty blank defaults, physical-centre changed-lattice lock review, headless native normal/narrow verification, maximum-grid diagnostics, and the standalone Windows publication boundary. Shared static canvas brushes/pens are immutable so multiple native windows and test UI threads can render safely.
